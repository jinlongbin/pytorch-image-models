...
__all__ = [
    'caitjin_M48', 'caitjin_M36',
    'caitjin_S36', 'caitjin_S24','caitjin_S24_224',
    'caitjin_XS24','caitjin_XXS24','caitjin_XXS24_224',
    'caitjin_XXS36','caitjin_XXS36_224'
]
...
class LayerScale_Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 ...
                 Mlp_block=Mlp,init_values=1e-4, n=196):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = Attention_block(
            dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop)
        ...
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = Mlp_block(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
        self.gamma_1 = nn.Parameter(init_values * torch.ones((n)),requires_grad=True)
        self.gamma_2 = nn.Parameter(init_values * torch.ones((n)),requires_grad=True)

    def forward(self, x, evec):        
        gamma_1 = torch.bmm((evec.permute(0, 2, 1) * self.gamma_1), evec)
        gamma_2 = torch.bmm((evec.permute(0, 2, 1) * self.gamma_2), evec)
        
        x = x + self.drop_path(torch.bmm(gamma_1, self.attn(self.norm1(x))))
        x = x + self.drop_path(torch.bmm(gamma_2, self.mlp(self.norm2(x))))
        return x 
...
class cait_models(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.evecs = self.laplacian_evec((img_size//patch_size)**2, depth).cuda()
        ...
        self.blocks = nn.ModuleList([
            block_layers(
                ...
                act_layer=act_layer,Attention_block=Attention_block,Mlp_block=Mlp_block,init_values=init_scale, n=self.n)
    ...
    def laplacian_evec(self, n, nhop):
        lm = torch.zeros((n, n))

        n2 = int(n**0.5)
        for i in range(1, n+1):
            if i == 1:
                neighb_index = [i+1, i+n2, i+n2+1]
            elif i == n2:
                neighb_index = [i-1, i+n2-1, i+n2]
            elif i == int(n - n2 + 1):
                neighb_index = [i+1, i-n2+1, i-n2]
            elif i == n:
                neighb_index = [i-1, i-n2-1, i-n2]
            elif 1 < i < n2:
                neighb_index = [i-1, i+1, i+n2-1, i+n2, i+n2+1]
            elif int(n-n2+1) < i < n:
                neighb_index = [i-1, i+1, i-n2+1, i-n2, i-n2-1]
            elif i % n2 == 0 and i not in (n, n2):
                neighb_index = [i-1, i+n2-1, i+n2, i-n2, i-n2-1]
            elif i % n2 == 1 and i not in (1, n-n2+1):
                neighb_index = [i+1, i-n2, i-n2+1, i+n2, i+n2+1]
            else:
                neighb_index = [i-1, i+1, i+n2-1, i+n2, i+n2+1, i-n2+1, i-n2, i-n2-1]

            lm[i-1,i-1] = len(neighb_index)
            for j in neighb_index:
                lm[i-1,j-1] = -1    
        
        evecs = torch.zeros((nhop, n, n))
        eval, evec = torch.eig(lm, eigenvectors=True)
        evecs[0,:,:] = evec

        lm_ = lm.clone()
        for i in range(1, nhop):
            lm_ = torch.matmul(lm_, lm)
            eval, evec = torch.eig(lm, eigenvectors=True)
            evecs[i,:,:] = evec
...
@register_model
def caitjin_XXS24_224(pretrained=False, **kwargs):
    model = cait_models(
...
@register_model
def caitjin_XXS24(pretrained=False, **kwargs):
    model = cait_models(
    ...
    return model 
@register_model
def caitjin_XXS36_224(pretrained=False, **kwargs):
    model = cait_models(
...
@register_model
def caitjin_XXS36(pretrained=False, **kwargs):
    model = cait_models(
...
@register_model
def caitjin_XS24(pretrained=False, **kwargs):
    model = cait_models(
...
@register_model
def caitjin_S24_224(pretrained=False, **kwargs):
    model = cait_models(
...
@register_model
def caitjin_S24(pretrained=False, **kwargs):
    model = cait_models(
...
@register_model
def caitjin_S36(pretrained=False, **kwargs):
    model = cait_models(
...
@register_model
def caitjin_M36(pretrained=False, **kwargs):
    model = cait_models(
...
@register_model
def caitjin_M48(pretrained=False, **kwargs):
    model = cait_models(
